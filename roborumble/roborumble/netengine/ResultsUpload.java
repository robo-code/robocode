	...
	public boolean uploadResults() {
		...
		try {
			...
			while ( (record=br.readLine()) != null ) { 
				if (record.indexOf(game)!=-1) { match = record; status = 0; }
				else if (status == 0) { bot1 = record; status = 1; }
				else if (status == 1) { bot2 = record; results.add(match); results.add(bot1); results.add(bot2); }
			}
			br.close();
		} catch (Exception e) { System.out.println("Can't open result file for upload"); return false; }
			
		//Open the temp file to put the unuploaded results
		PrintStream outtxt = null;
		try { outtxt = new PrintStream(new BufferedOutputStream(new FileOutputStream(tempdir+"results.txt")),false); } 
		catch (IOException e) { 
		...
		try { battlesnum = new PrintStream(new BufferedOutputStream(new FileOutputStream(battlesnumfile)),false); } 
		catch (IOException e) { 
		...
		try { prioritybattles = new PrintStream(new BufferedOutputStream(new FileOutputStream(priority)),false); } 
		catch (IOException e) { 
		...
		for (int i=0; i<results.size()/3; i++) {
			...
			//find the match mode
			String matchtype = "GENERAL"; if (header.length >= 6)  matchtype = header[5];
			...
			String	data = "version=1"+"&"+"game="+game+"&"+"rounds="+header[1]+"&"+"field="+header[2]+"&"+"user="+user+"&"+"time="+header[4]+"&"+"fname="+first[0]+"&"+"fscore="+first[1]+"&"+"fbulletd="+first[2]+"&"+"fsurvival="+first[3]+"&"+"sname="+second[0]+"&"+"sscore="+second[1]+"&"+"sbulletd="+second[2]+"&"+"ssurvival="+second[3];
			if (matchtype.equals("GENERAL")||matchtype.equals("SERVER")) errorsfound = errorsfound | senddata(game,data,outtxt,true,results,i,battlesnum,prioritybattles);
			
			if (!sizesfile.equals("")) { //upload also related competitions
				if (!minibots.equals("") && !matchtype.equals("NANO")  && !matchtype.equals("MICRO")  && size.CheckCompetitorsForSize(first[0],second[0],1500)) {
					data = "version=1"+"&"+"game="+minibots+"&"+"rounds="+header[1]+"&"+"field="+header[2]+"&"+"user="+user+"&"+"time="+header[4]+"&"+"fname="+first[0]+"&"+"fscore="+first[1]+"&"+"fbulletd="+first[2]+"&"+"fsurvival="+first[3]+"&"+"sname="+second[0]+"&"+"sscore="+second[1]+"&"+"sbulletd="+second[2]+"&"+"ssurvival="+second[3];
					errorsfound = errorsfound | senddata(minibots,data,outtxt,false,results,i,battlesnum,null);
				}
				if (!microbots.equals("") && !matchtype.equals("NANO")  && size.CheckCompetitorsForSize(first[0],second[0],750)) {
					data = "version=1"+"&"+"game="+microbots+"&"+"rounds="+header[1]+"&"+"field="+header[2]+"&"+"user="+user+"&"+"time="+header[4]+"&"+"fname="+first[0]+"&"+"fscore="+first[1]+"&"+"fbulletd="+first[2]+"&"+"fsurvival="+first[3]+"&"+"sname="+second[0]+"&"+"sscore="+second[1]+"&"+"sbulletd="+second[2]+"&"+"ssurvival="+second[3];
				...
				if (!nanobots.equals("") && size.CheckCompetitorsForSize(first[0],second[0],250)) {
					data = "version=1"+"&"+"game="+nanobots+"&"+"rounds="+header[1]+"&"+"field="+header[2]+"&"+"user="+user+"&"+"time="+header[4]+"&"+"fname="+first[0]+"&"+"fscore="+first[1]+"&"+"fbulletd="+first[2]+"&"+"fsurvival="+first[3]+"&"+"sname="+second[0]+"&"+"sscore="+second[1]+"&"+"sbulletd="+second[2]+"&"+"ssurvival="+second[3];
		...
		boolean b = r.delete();
		if (!b) System.out.println("Unable to delete results file.");

		//copy temp file into results file if there was some error
		if (errorsfound) {
			try { Utils.copy(new File(tempdir+"results.txt"), new File(resultsfile)); }
			catch (Exception e) { System.out.println("Error when copying results errors file."); }
		}
		...
		try {
			...
			while ((line = rd.readLine()) != null) {
				if (line.indexOf("OK")!=-1) {
					...
					String bot1 = (String)results.get(i*3+1); bot1 = bot1.substring(0,bot1.indexOf(","));
					String bot2 = (String)results.get(i*3+2); bot2 = bot2.substring(0,bot2.indexOf(","));
					...
					line = line.substring(1); line = line.substring(0,line.length()-1);
					...
					if (prioritybattles != null) prioritybattles.println(battle);
			...
			rd.close();
			if (!ok) { 
				saverror(outtxt, (String)results.get(i*3), (String)results.get(i*3+1),(String)results.get(i*3+2), saveonerror); 
				if (saveonerror) errorsfound = true; 
			}
		} catch (Exception e) { 
			System.out.println(e);
			if (saveonerror) errorsfound = true; 

...
public class BotsDownload {
	...
	public BotsDownload(String propertiesfile) {
		...
		generalbots = propertiesfile; while (generalbots.indexOf("/")!=-1) generalbots = generalbots.substring(generalbots.indexOf("/")+1); generalbots=generalbots.substring(0,generalbots.indexOf("."));
	...
	public boolean downloadRatings() {
		...
		if (!generalbotsfile.equals("")) (new File(generalbotsfile)) .delete();
		if (!minibotsfile.equals("")) (new File(minibotsfile)) .delete();
		if (!microbotsfile.equals("")) (new File(microbotsfile)) .delete();
		if (!nanobotsfile.equals("")) (new File(nanobotsfile)) .delete();
		//download new ones
		if (ratingsurl.equals("")) return false;
		boolean downloaded = true;
		if (!generalbots.equals("") && !generalbotsfile.equals("")) downloaded = downloadRatingsFile(generalbots,generalbotsfile) & downloaded;
		if (!minibots.equals("") && !minibotsfile.equals("")) downloaded = downloadRatingsFile(minibots,minibotsfile) & downloaded;
		if (!microbots.equals("") && !microbotsfile.equals("")) downloaded = downloadRatingsFile(microbots,microbotsfile)  & downloaded;
		if (!nanobots.equals("") && !nanobotsfile.equals("")) downloaded = downloadRatingsFile(nanobots,nanobotsfile) & downloaded;
		return downloaded;
	}
	...
	private boolean downloadBot(String botname, String file,String id, String destination, String tempdir) {
		...
		URL url = null;
		try { 
		    if (id.indexOf("://")==-1) url = new URL("http://www.robocoderepository.com/Controller.jsp?submitAction=downloadClass&id="+id); 
		    else url = new URL(id);
		}
		catch (Exception e) { System.out.println("Wrong URL"); return false; }

		System.out.println("Downloading ..." + botname);	
		boolean downloaded = filetransfer.download(url,new File(filed));
		if (!downloaded) { System.out.println("Unable to download "+botname+" from site."); return false; }

		//Check the bot and save it into the repository
			
		if (checkJarFile(filed,botname)) {
			try { Utils.copy(new File(filed), new File(finald)); }
			catch (Exception e) { System.out.println("Unable to copy "+filed+" into the repository"); return false; }
			System.out.println("Downloaded "+botname+" into "+finald);
		} else { System.out.println("Downloaded file is wrong or corrupted:"+file); return false; } 
	...
	private boolean checkJarFile(String file, String botname) {
		...
		bot = bot.replace('.','/'); 
		if (!isteams.equals("YES")) { bot = bot + ".properties"; } 
		else { bot = bot + ".team"; }
		...
		try {
			...
			if (!isteams.equals("YES")) {
				...
				if (botname.equals(botname.substring(0,botname.indexOf(" "))+" "+version)) return true; else return false;				
			}
	...
	public boolean notifyServerForOldParticipants() {
		...
		try {
			...
			while ( (record=br.readLine()) != null ) { 
				if (record.indexOf(",") != -1) {
					String name = record.substring(0,record.indexOf(",")); name =name.replace(' ','_');
					namesall.put(name,name);
				}
			}
			br.close();
		} catch (Exception e) { System.out.println("Participants file not found when removing old participants ... Aborting"); System.out.println(e); return false; }
		...
		try { generalratings.load(new FileInputStream(generalbotsfile)); } catch (Exception e) { generalratings = null; }
		try { miniratings.load(new FileInputStream(minibotsfile)); } catch (Exception e) { miniratings = null; }
		try { microratings.load(new FileInputStream(microbotsfile)); } catch (Exception e) { microratings = null; }
		try { nanoratings.load(new FileInputStream(nanobotsfile)); } catch (Exception e) { nanoratings = null; }
		
		//Check general ratings	
		if (generalratings == null) return false;
		...
		//Check mini ratings	
		if (miniratings == null) return true;
		...
		//Check micro ratings
		if (microratings == null) return true;
		...
		//Check nano ratings
		if (nanoratings == null) return true;
	...
	private void removebot(String game, String bot) {
		if (removeboturl.equals("")) { System.out.println("UPDATEBOTS URL not defined!"); return; }
		...
		try {
			...
			String line; while ((line = rd.readLine()) != null) { System.out.println(line); }

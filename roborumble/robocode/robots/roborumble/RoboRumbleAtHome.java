...
public class RoboRumbleAtHome  {
	...
	public static void main(String args[]) {
		...
		try { parameters = args[0];  }
		catch(Exception e) { System.out.println("No argument found specifying properties file. \"roborumble.txt\" assumed."); }

		//Read parameters for running the app
		Properties param = null;
		try { param = new Properties(); param.load(new FileInputStream(parameters)); } 
		...
		do {			
			...
			if (executes.equals("YES")) {
				...
				if (melee.equals("YES")) {
					...
					System.out.println("Preparing battles list ... Using smart battles is "+(ratingsdownloaded && runonly.equals("SERVER")));
					if (ratingsdownloaded && runonly.equals("SERVER")) ready = battles.createSmartBattlesList(); //create the smart lists
					else ready = battles.createBattlesList(); //create the normal lists 
				}

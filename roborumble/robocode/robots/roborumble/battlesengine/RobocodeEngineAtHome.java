...
public class RobocodeEngineAtHome {
    ...
    public RobocodeEngineAtHome(RobocodeListener listener) {
		...
		if (robotsDir.exists()) init(robocodeDir, listener);
		else throw new RuntimeException("File not found: " + robotsDir);
    }
	...
	for (ThreadGroup tg = Thread.currentThread().getThreadGroup();
	...
	public boolean isBattleRunning() { return manager.getBattleManager().isBattleRunning(); }

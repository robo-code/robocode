...
public class BattleManager implements IBattleManager {
	...
	public void startNewBattle(BattleSpecification spec,
		...
		battleProperties
				.setBattlefieldHeight(spec.getBattlefield().getHeight());
	...
	private void startNewBattleImpl(RobotSpecification[] battlingRobotsList,
		...
		//F13 used only in case of the BattleRunner when saveURL is set to something
		if(saveURL!=null)
		this.recordManager.attachRecorder(battleEventDispatcher);
		...
		System.out.println("Checking for the End. . . . .");
		
		if(saveURL!=null)
		this.recordManager.saveRecord(saveURL, BattleRecordFormat.BINARY_ZIP);
	}

	/**
	 * This function retrieves the extension information from external jars
	 * through battleProperties. The CustomRulesApi is also loaded at this
	 * point.
	 * 
	...
	private CustomRules getCustomRules() {
		...
		game_mode = properties.getProperty("GAMEMODE", "CaptureTheFlag");
		if(game_mode.equalsIgnoreCase("NORMALMODE"))
		{
			...
			try {
				// TODO: For convenience, I've hardcoded the extension package
				// strings here.
				// This should be cleaned later.
				String extensionPackagename = "CTF";
				//String extensionPackagename = "net.sf.robocode.extensions";
				...
				String extensionFilename = "robocodeextensionsalpha.jar";
				//extensionFilename = battleProperties.getExtensionFilename();
	
				// Make sure engineLoader knows about the extension being used
				File jarFile = new File(FileUtil.getExtensionsDir(),
						extensionFilename);
				((EngineClassLoader) Container.engineLoader).addURL(jarFile.toURI()
						.toURL());
				// Register extension custom rules with the engine loader
				Container.loadJars(FileUtil.getExtensionsDir());
	
				Class<?> loadedCustomRules = (Class<?>) Class.forName(
						extensionPackagename + "." + extensionClassname, true,
						Container.engineLoader);
	
				CustomRules newInstance = (CustomRules) loadedCustomRules
						.newInstance();
	...
	public String getBattleFilename() {
		...
		if (filename != null) {
			if (filename.indexOf(File.separatorChar) < 0) {
				filename = FileUtil.getBattlesDir().getName()

 ...
 *     - Refactoring
 *     Himanshu Singh
 *		- modifications for VirtualCombat
 *******************************************************************************/
...
public abstract class BaseBattle implements IBattle, Runnable {
	...
	private final static int TURNS_DISPLAYED_AFTER_ENDING = 25;
	...
	public void run() {
		try {
			int abort = initializeBattle();
			
			if (abort == 1) {
				System.out.println("Aborting battle now.");
				//sendCommand(new AbortCommand());
				isAborted=true;
				JOptionPane.showMessageDialog(null, "Battle cannot take place between more than two teams !");
			}
	...
	protected int initializeBattle() {
	...
	/* Himanshu Singh */	
	while (command != null && checkGameOver()==false) {/* Team Himanshu */
			try {
				command.execute();
			} catch (Exception e) {
				logError(e);

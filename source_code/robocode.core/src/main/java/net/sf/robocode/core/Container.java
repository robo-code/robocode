...
public final class Container extends ContainerBase {
	...
	//public static String calledFrom = "BattleManager";
	...
	public static void loadJars(File pathf) {
		...
		for (File file : files) {
			try {
				if(file.toString().contains("robocodeextensionsapi.jar"))
				{
					loadModule(getModuleName(file.toString()), systemLoader);
				}
				else{
					loadModule(getModuleName(file.toString()), engineLoader);
				}
			} catch (NullPointerException e) {
				System.out.println("loadJars : caught the null pointer exception");
			}
		}
	}
	...
	private static boolean loadModule(String module, ClassLoader loader) {
		try {
			...
			Class<?> modClass;
			if (module.contains("CTFApi"))		//hack by Himanshu
			{
				//System.out.println("Loading ICTFApi in loader = "+ loader.toString());
				modClass = loader.loadClass("CTFApi.ICaptureTheFlagApi");
				//System.out.println("Loading CTFApi in loader = "+ loader.toString());
				modClass = loader.loadClass("CTFApi.CaptureTheFlagApi");
				
			}
			else{
				if (module.contains("CTF"))	//More hacks - Josh
				{
					System.out.println("Loading CTF rules");
	...
	private static String getModuleName(String path) {
		...
		if (test.endsWith("extensionsapi.jar"))		//hack by Himanshu
		{
			return "CTFApi.CaptureTheFlagApi";
		}

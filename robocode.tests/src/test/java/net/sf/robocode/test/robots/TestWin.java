...
public class TestWin extends RobocodeTestBed {
	...
	protected void runTeardown() {
		// System.out.print(outputBuf.toString());
		...
		Assert.assertThat(lines[12], is("SYSTEM: Bonus for killing sample.MyFirstRobot: 1"));
		Assert.assertThat(lines[14], is("Round 3 of 5"));
		Assert.assertThat(lines[16], is("RoundEnded!"));
		Assert.assertThat(lines[17], is("SYSTEM: tested.robots.BattleWin has died"));
		Assert.assertThat(lines[18], is("Death!"));
		...
		Assert.assertThat(lines[29], is("SYSTEM: Bonus for killing sample.MyFirstRobot: 4"));
		Assert.assertThat(lines[30], is("SYSTEM: tested.robots.BattleWin wins the round."));
		Assert.assertThat(lines[31], is("RoundEnded!"));
		Assert.assertThat(lines[32], is("Win!"));
		Assert.assertThat(lines[33], is("BattleEnded!"));

		Assert.assertThat("1st robot should get right score", results[0].getScore(), is(156));
		Assert.assertThat("2nd robot should get right score", results[1].getScore(), is(148));
	}
}

...
public interface IBasicRobotPeer {
	 ...
	 * {@link robotapi.Rules#MAX_VELOCITY} (8 pixels / turn).
	 *
	 * @return the velocity of the robot measured in pixels/turn.
	 * @see robotapi.Rules#MAX_VELOCITY
	 */
	double getVelocity();
	 ...
	 *                 finish its turn.
	 * @see robotapi.robotinterfaces.IBasicEvents#onHitWall(robotapi.HitWallEvent)
	 ...
	 * {@link robotapi.Rules#getBulletDamage(double)} for getting the damage that a
	 * bullet with a specific bullet power will do.
	 * <p/>
	 * The specified bullet power should be between
	 * {@link robotapi.Rules#MIN_BULLET_POWER} and {@link robotapi.Rules#MAX_BULLET_POWER}.
	 * <p/>
	 * Note that the gun cannot fire if the gun is overheated, meaning that
	 * {@link #getGunHeat()} returns a value > 0.
	 * <p/>
	 * A event is generated when the bullet hits a robot
	 * ({@link robotapi.BulletHitEvent}), wall ({@link robotapi.BulletMissedEvent}), or another
	 * bullet ({@link robotapi.BulletHitBulletEvent}).
	 * <p/>
	 * Example:
	 * <pre>
	 ...
	 * @param power the amount of energy given to the bullet, and subtracted
	 *              from the robot's energy.
	 * @return a {@link robotapi.Bullet} that contains information about the bullet if it
	 *         was actually fired, which can be used for tracking the bullet after it
	 *         has been fired. If the bullet was not fired, {@code null} is returned.
	 * @see #fire(double)
	 * @see robotapi.Bullet

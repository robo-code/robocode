...
//	boolean changed = false;
	...
	String externalRobotsPath = manager.getProperties().getOptionsDevelopmentPath();
	{
		...
		while (tokenizer.hasMoreTokens())
		{
			...
			File f = new File(tok);
			if (!f.equals(getRobotsDirectory()) && !f.equals(getRobotCache()) && !f.equals(getRobotsDirectory().getParentFile()))
...
//	FileSpecificationVector fileSpecificationVector = getRobotDatabase().getFileSpecifications();
	...
	if (cacheWarning)
	{
		JOptionPane.showMessageDialog(null,
			...
			"Unexpected robotcache update", JOptionPane.OK_OPTION);
	}
	...
	if (files != null)
	{
		for (int i = 0; i < files.length; i++)
		{
			if (files[i].isDirectory() && files[i].getName().lastIndexOf(".jar_") == files[i].getName().length()-5 ||
				files[i].isDirectory() && files[i].getName().lastIndexOf(".zip_") == files[i].getName().length()-5 ||
				files[i].isDirectory() && files[i].getName().lastIndexOf(".jar") == files[i].getName().length()-4)
			{
				File f = new File(getRobotsDirectory(),files[i].getName().substring(0,files[i].getName().length()-1));
...
private FileSpecificationVector getSpecificationsInDirectory(File rootDir, File dir, String prefix, boolean isDevelopmentDirectory) {
	...
	for (int i = 0; i < files.length; i++)
	{
			...
			if (prefix.equals(""))
			{
				...
				else
				{
					...
					if (jidx > 0 && jidx == files[i].getName().length()-5)
					{
						...
						robotList.add(getSpecificationsInDirectory(rootDir, files[i],prefix + files[i].getName() + ".",isDevelopmentDirectory));
				}
			}
			else
			{
				...
				if (didx > 0 && didx == files[i].getName().length()-5)
					continue; // Don't process .data dirs
				robotList.add(getSpecificationsInDirectory(rootDir, files[i],prefix + files[i].getName() + ".",isDevelopmentDirectory));
			}
		}
		else if (files[i].getName().indexOf("$") < 0 && files[i].getName().indexOf("robocode") != 0)
		{
			...
			if (cachedSpecification != null && cachedSpecification.isSameFile(files[i].getPath(),files[i].length(),files[i].lastModified()))
			{
//				log("This file is unchanged.");
			...
			//		log("This file has changed.");
				fileSpecification = FileSpecification.createSpecification(this,files[i],rootDir,prefix,isDevelopmentDirectory);
				updateRobotDatabase(fileSpecification);
				write = true;
				if (fileSpecification instanceof JarSpecification)
				{
...
private void updateRobotDatabase(FileSpecification fileSpecification) {
	...
	if (fileSpecification instanceof RobotSpecification)
	{
		...
		try {
			...
			Class robotClass = robotClassManager.getRobotClassLoader().loadRobotClass(robotClassManager.getFullClassName(),true);
...
private void updateNoDuplicates(FileSpecification spec)
{
	...
	if (!spec.isDevelopmentVersion() && getRobotDatabase().contains(spec.getFullClassName(),spec.getVersion(),false))
	{
			...
			else //if (duplicatePrompt)
			{
				...
				if (existingSource != null && newSource != null)
				{
					...
					if (t1 > t2)
					{
						...
						getRobotDatabase().put(key,spec);
						conflictLog("Renaming " + existingSource + " to invalid, as it contains a robot " + spec.getName() + " which conflicts with the same robot in " + newSource);
					}
					else
					{
						newSource.renameTo(new File(newSource.getPath() + ".invalid"));
						conflictLog("Renaming " + newSource + " to invalid, as it contains a robot " + spec.getName() + " which conflicts with the same robot in " + existingSource);
					}
				}
...
/*				if (entry.getName().indexOf("/") < 0 && Utils.getFileType(entry.getName()).equals(".jar"))
				...
				*/
					...
					if (entry.getName().indexOf("/") < 0 && Utils.getFileType(entry.getName()).equals(".jar"))
					{
						FileSpecification fileSpecification = FileSpecification.createSpecification(this,out,parentDirectory,"",false);
						updatedJarVector.add(fileSpecification);
					}
...
public boolean verifyRootPackage(String robotName) {
		...
		if (lIndex > 0)
		{
			...
			{
				if (robotName.equals("sample.Corners")) return true;
				if (robotName.equals("sample.Crazy")) return true;
				if (robotName.equals("sample.Fire")) return true;
				if (robotName.equals("sample.MyFirstRobot")) return true;
				if (robotName.equals("sample.RamFire")) return true;
				if (robotName.equals("sample.SittingDuck")) return true;
				if (robotName.equals("sample.SpinBot")) return true;
				if (robotName.equals("sample.Target")) return true;
				if (robotName.equals("sample.TrackFire")) return true;
				if (robotName.equals("sample.Tracker")) return true;
				if (robotName.equals("sample.Walls")) return true;

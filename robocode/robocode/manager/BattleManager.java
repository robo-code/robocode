...
public void startNewBattle(BattleProperties battleProperties, boolean exitOnComplete) {
	...
	FileSpecificationVector robotSpecificationsVector = manager.getRobotRepositoryManager().getRobotRepository().getRobotSpecificationsVector(false,false,false,false,false,false);
	...
	if (battleProperties.getSelectedRobots() != null)
	{
		...
		while (tokenizer.hasMoreTokens())
		{
			...
			{
				FileSpecification currentFileSpecification = (FileSpecification)robotSpecificationsVector.elementAt(i);
...
public void startNewBattle(robocode.control.BattleSpecification battleSpecification) {
	...
	for (int i = 0; i < robotSpecs.length; i++)
	{
		...
		for (int j = 0; j < robotSpecificationsVector.size(); j++)
		{
			...
			// " to " + bot);
			if (((FileSpecification)robotSpecificationsVector.elementAt(j)).getNameManager().getUniqueFullClassNameWithVersion().equals(bot))
			{
...
private void startNewBattle(RobotClassManagerVector battlingRobotsVector, boolean exitOnComplete, robocode.control.BattleSpecification battleSpecification)
{
	...
	*/
	...
	if (battle != null)
	{
		...
		{
			//bugFixLoaderCache.add(battle.getRobots().elementAt(i).getRobotClassManager().getRobotClassLoader());
		}
		battle.stop();
	}

	BattleField battleField = 
	  new ShapesBattleField(battleProperties.getBattlefieldWidth(),

...
public class Battle implements Runnable {
	...
	public void run() {
		...
		if (unsafeLoadRobotsThread != null
			&& Thread.currentThread() == unsafeLoadRobotsThread) {
			unsafeLoadRobots();
			return;
		}
		...
		while (!abortBattles && getRoundNum() < getNumRounds()) {
			//		battleActiveTime = System.currentTimeMillis();
			battleView.setTitle(
				"Robocode: Starting Round " + (roundNum + 1) + " of " + numRounds);
			try {
				setupRound();
	...
	public void addRobot(RobotClassManager robotClassManager) {
		...
		RobotPeer robotPeer =
			new RobotPeer(
				robotClassManager,
				battleManager.getManager().getRobotRepositoryManager(),
		...
		int count = 0;
		for (int i = 0; i < robots.size(); i++) {
			if ((robots.elementAt(i))
				.getRobotClassManager()
				.getClassNameManager()
				.getFullClassNameWithVersion()
				.equals(
					robotPeer
						.getRobotClassManager()
						.getClassNameManager()
	...
	public void initialize() {
		...
		for (int i = 0; i < robots.size(); i++) {
			...
			try {
				...
				Class c =
					r.getRobotClassManager().getRobotClassLoader().loadRobotClass(className, true);
				...
				*/
				...
				for (int j = 0; j < 1000; j++) {
					...
					y =
						r.getHeight() + Math.random() * (battleField.getHeight() - 2 * r.getHeight());
		...
		if (battleManager.isPaused() && abortBattles == false) {
			if (!wasPaused) {
				if (roundNum < numRounds) {
					battleView.setTitle(
						"Robocode: Round " + (roundNum + 1) + " of " + numRounds + " (paused)");
				} else {
			...
			for (int i = 0; i < robots.size(); i++) {
				...
				for (int j = 0; j < deathEvents.size(); j++) {
					r.getEventManager().add(
						new RobotDeathEvent(((RobotPeer) deathEvents.elementAt(j)).getName()));
					if (r.getTeamPeer() == null
						|| r.getTeamPeer() != deathEvents.elementAt(j).getTeamPeer())
		...
		for (int i = 0; i < deathEvents.size(); i++) {
			...
			if (r.getTeamPeer() == null)
				...
				for (int j = 0; j < robots.size(); j++) {
					if (robots.elementAt(j).getTeamPeer() == r.getTeamPeer()
						&& (!robots.elementAt(j).isDead())) {
						teammatesalive = true;
						break;
					}
				}
	...
	public void setupRound() {
		...
		for (int i = 0; i < robots.size(); i++) {
			r = (RobotPeer) robots.elementAt(i);
			if (roundNum > 0)
				r.preInitialize(); // fake dead so robot won't display
		...
		*/
		for (int i = 0; i < robots.size(); i++) {
			...
			if (r.getRobotClassManager().getClassNameManager().getFullPackage() != null
				&& r.getRobotClassManager().getClassNameManager().getFullPackage().length()
					> 18) {
				r.out.println(
					"SYSTEM: Your package name is too long.  16 characters maximum please.");
				r.out.println("SYSTEM: Robot disabled.");
				r.setEnergy(0);
			}
			if (r.getRobotClassManager().getClassNameManager().getShortClassName().length()
				> 35) {
				r.out.println(
					"SYSTEM: Your classname is too long.  32 characters maximum please.");
				r.out.println("SYSTEM: Robot disabled.");
				r.setEnergy(0);
			}
		}
		...
		for (int i = 0; i < robots.size(); i++) {
			r = (RobotPeer) robots.elementAt(i);
			manager.getThreadManager().addThreadGroup(
				r.getRobotThreadManager().getThreadGroup(),
				r);
		...
		while (true) {
			...
			for (int i = 0; i < robots.size(); i++) {
				...
				try {
					...
					r.out.println(
						"SYSTEM: An error occurred during initialization of "
							+ r.getRobotClassManager());
		...
		for (int i = 0; i < robots.size(); i++) {
			...
			if (!currentRobot.isDead()) {
				if (!found) {
					...
					if (currentTeam == null
						&& currentRobot.getRobotClassManager().getTeamManager() == null)
						return false;

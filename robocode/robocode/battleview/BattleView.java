...
protected void drawObjects(Graphics2D osg)
{
	//((ShapesBattleField)battleField).drawShapes((Graphics2D)osg);
}
	...
	for (int i = 0; i < battle.getRobots().size(); i++)
	{
			...
			if (drawRobotEnergy && c.getRobot() != null)
			{
				...
				int ll = (int)c.getEnergy();
	 			...
	 			centerString(
		 			energyString,
	  				x,
	  				y - imgHeight / 2 - smallFontMetrics.getHeight() / 2,
	  				smallFont,
	  				smallFontMetrics,
	  				energyStringRect);
			}
		...
		if (c.getSayTextPeer() != null)
		{
			if (c.getSayTextPeer().getText() != null)
			{
				...
				centerString(
  					c.getSayTextPeer().getText(),
  					c.getSayTextPeer().getX(),
  					battle.getBattleField().getHeight()-c.getSayTextPeer().getY(),
  					smallFont,
  					smallFontMetrics,
...
//					osg.drawImage(bulletImage,l,t, null);
			...
			{
			    osg.drawImage(imageManager.getBulletImage(),l,t,(int)(1/scale + 1.0),(int)(1/scale + 1.0),null);
			}
			...
			*/
			...
			try {
				drawExplodeImage(osg,imageManager.getExplosionImage(b.getWhichExplosion(),b.getFrame(),sizeIndex),l,t);
			} catch (ArrayIndexOutOfBoundsException e) {
				//log("Out of bounds: " + b.getFrame() + " of size " + sizeIndex);
			}

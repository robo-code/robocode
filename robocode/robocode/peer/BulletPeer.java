		...
		for (int i = 0; b != this && i < battle.getBullets().size(); i++) {
			...
			if (b != null && b != this && b.isActive()) {
				...
				if (boundingLine.intersectsLine(b.getBoundingLine())
						&& !boundingLine.getP1().equals(
								b.getBoundingLine().getP2())
						&& !boundingLine.getP2().equals(
								b.getBoundingLine().getP1())) {
					active = false;
					b.active = false;
					hitBullet = true;
					hitBulletTime = 0;
					...
					setX(lastX);
					setY(lastY);
					owner.getEventManager()
							.add(
									new BulletHitBulletEvent(getBullet(), b
											.getBullet()));
					b.getOwner().getEventManager()
							.add(
									new BulletHitBulletEvent(b.getBullet(),
											getBullet()));
					break;
				}
			}
			...
			if (r != null && r != owner && !r.isDead()) {
				...
				if (r.getBoundingBox().intersectsLine(boundingLine)) {
					...
					if (r.getEnergy() <= 0) {
						if (!r.isDead()) {
							r.setDead(true);
							owner.getRobotStatistics()
									.scoreKilledEnemyBullet(i);
						}
					}
					...
					r.getEventManager().add(
							new HitByBulletEvent(robocode.util.Utils
									.normalRelativeAngle(heading + Math.PI
											- r.getHeading()), getBullet()));
					owner.getEventManager().add(
							new BulletHitEvent(r.getName(), r.getEnergy(),
									bullet));
					active = false;
					hitVictim = true;
					hitVictimTime = 0;

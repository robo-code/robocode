...
/*				double angleToHim =
		...
		*/		
				...
				//	distanceRemaining += velocity;
		   		...
		   		{
			   		//out.println("I'm moving: " + distanceRemaining + " with bearing: " + Math.toDegrees(bearing));
		   		}

 	 			eventManager.add(new HitRobotEvent(r.getName(),Utils.normalRelativeAngle(angle - heading),r.getEnergy(),atFault));
  				r.getEventManager().add(new HitRobotEvent(getName(),Utils.normalRelativeAngle(Math.PI + angle - r.heading),getEnergy(),false));
...
public void checkWallCollision() 
{
	...
	{
		hitWall = true;
		fixx = battleField.getBoundingBox().x + battleField.getBoundingBox().width - boundingBox.width - boundingBox.x - .001;
	...
	{
		hitWall = true;
		fixy = battleField.getBoundingBox().y + battleField.getBoundingBox().height - getBoundingBox().height - getBoundingBox().y - .001;
...
{
	if (rect.intersectsLine(arc.getCenterX(),arc.getCenterY(),arc.getStartPoint().getX(),arc.getStartPoint().getY()))
		return true;
...
public void scan() {
	...
	scanArc.setArc(x - scanRadius,y-scanRadius,2*scanRadius,2*scanRadius,180.0 * startAngle / Math.PI, 180.0 * scanRadians / Math.PI, java.awt.geom.Arc2D.PIE);
...
public final synchronized void update()  {
	...
	if (scan == false)
	{
		if (lastHeading != heading || lastGunHeading != gunHeading || lastRadarHeading != radarHeading ||
...
public void setDuplicate(int count) {
	...
	this.shortName = getRobotClassManager().getClassNameManager().getUniqueShortClassNameWithVersion() + " (" + (count + 1) + ")";

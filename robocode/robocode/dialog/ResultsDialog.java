...
*/
			tableSize = new Dimension(
				getResultsTable().getColumnModel().getTotalColumnWidth(),
				getResultsTable().getModel().getRowCount() * (getResultsTable().getRowHeight() + getResultsTable().getRowMargin())
				);
...
private void setResultsData() {
	...
	for (int x = 0; x < getBattleResultsTableModel().getColumnCount(); x++)
	{
		...
		{
			comp = getResultsTable().getDefaultRenderer(
				getBattleResultsTableModel().getColumnClass(x)

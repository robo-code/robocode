...
public AvailableRobotsPanel getAvailableRobotsPanel() {
	if (availableRobotsPanel == null) {
		try {
			availableRobotsPanel = new AvailableRobotsPanel(getAddButton(),"Available Robots",getSelectedRobotsList(),this);
...
private void selectedRobotsListSelectionChanged() {
	...
	{
		availableRobotsPanel.clearSelection();
		FileSpecification robotSpecification = (FileSpecification)getSelectedRobotsList().getModel().getElementAt(sel[0]);
...
private void setSelectedRobots(FileSpecificationVector robotList, String selectedRobotsString) {
	if (selectedRobotsString != null)
	{
		...
		while (tokenizer.hasMoreTokens())
		{
			...
			{
				if (((FileSpecification)robotList.elementAt(i)).getNameManager().getUniqueFullClassNameWithVersion().equals(bot))
				{

...
public static RobocodeCompiler createCompiler(RobocodeEditor editor) {
	...
	if (getCompilerProperties().getCompilerBinary() == null || getCompilerProperties().getCompilerBinary().equals(""))
	{
		if (installCompiler(editor))
			return new RobocodeCompiler(editor,
									getCompilerProperties().getCompilerBinary(),
			 						getCompilerProperties().getCompilerOptions(),
			 						getCompilerProperties().getCompilerClasspath());
		else
		{
...
//	log(System.getProperty("java.ext.dirs"));
	...
	{
		javalib = new File(System.getProperty("java.home")).getParentFile().getPath() + File.separator + "Classes" + File.separator + "classes.jar";
		if (javalib.indexOf(" ") > 0)
			javalib = "\"" + javalib + "\"";
		jikesJar = "compilers/jikes-1.16.src.jar";
	...
	if (javacOk)
	{
		...
		int rc = JOptionPane.showConfirmDialog(editor,"Robocode has found a working javac on this system.\nWould you like to use it?\n" + noString,"Confirm javac",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
	...
	else if (jikesOk)
	{
		...
		getCompilerProperties().setCompilerBinary(compilerBinary);
		getCompilerProperties().setCompilerOptions(compilerOptions);
		getCompilerProperties().setCompilerClasspath("-classpath " + javalib + File.pathSeparator + "robocode.jar" + File.pathSeparator + getRobotPath());
		saveCompilerProperties();
...
public static boolean testJavac(ConsoleDialog console) {
	...
	try {
		Process p = Runtime.getRuntime().exec("javac compilers" + File.separator + "CompilerTest.java",null,Constants.cwd());
		console.processStream(p.getInputStream());
		console.processStream(p.getErrorStream());
		p.waitFor();
		if (p.exitValue() == 0)
...
public static boolean testJikes(ConsoleDialog console, String jikesBinary) {
	...
	boolean jikesOk = false;
	try {
		Process p = Runtime.getRuntime().exec(jikesBinary + " compilers" + File.separator + "CompilerTest.java",null,Constants.cwd());
		console.processStream(p.getInputStream());
		console.processStream(p.getErrorStream());
		p.waitFor();
		if (p.exitValue() == 0)

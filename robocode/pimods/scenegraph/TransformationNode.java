...
import pimods.math.Transform3f;
...
//	}
	...
	public boolean removeDescendant( Drawable d){
		if( drawables.remove( d)) return true;
		else for( Drawable child : drawables){
			if( child instanceof TransformationNode){
				if( ((TransformationNode) child).removeDescendant( d)) return true;
			}
		}
		return false;
	}

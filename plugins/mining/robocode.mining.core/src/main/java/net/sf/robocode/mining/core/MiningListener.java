...
public class MiningListener extends BattleAdaptor {
            ...
            switch (bullet.getState()) {
                ...
                case MOVING:
                    ...
                    if (bulletAggregation.Last == null) {
                        ...
                        double bulletTraveled = Point2D.Double.distance(bulletAggregation.FireOwner.getX(), bulletAggregation.FireOwner.getY(), bullet.getX(), bullet.getY());
                        double victimDistance = Point2D.Double.distance(bulletAggregation.FireOwner.getX(), bulletAggregation.FireOwner.getY(), victim.getX(), victim.getY());
    ...
    public void Process(BulletAggregation bullet) {
        ...
        double[] preciseMeAs = PreciseMinMaxGFs.getPreciseMEAs(enemyLocation, enemy.getBodyHeading(),
                enemy.getVelocity(), myLocation, bullet.First.getPower(),
                rotationDirection, pointsList);
        ...
        bullet.OwnerFireAngle = Utils.normalRelativeAngle(PreciseMinMaxGFs.absoluteBearing(myLocation, enemyLocation) - bullet.First.getHeading());
        bullet.VictimEscapeAngle = Utils.normalRelativeAngle(PreciseMinMaxGFs.absoluteBearing(myLocation, enemyLocation) - PreciseMinMaxGFs.absoluteBearing(myLocation, victimEscape));
        ...
        System.out.println(
                "\"" + bullet.AimOwner.getName()
                        + "\" \"" + bullet.AimVictim.getName() + "\" "
                        + bullet.First.getBulletId() + " "
                        + (bullet.IsHit ? "H" : "M") + bullet.First.getPower()
                        + " O" + bullet.OwnerFireGF
                        + " V" + bullet.VictimEscapeGF
        );

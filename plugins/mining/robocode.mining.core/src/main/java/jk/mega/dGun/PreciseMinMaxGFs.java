...
public class PreciseMinMaxGFs {
    ...
    public static double[] getPreciseMEAs(
        ...
        for (int i = endPoints.size() - 1; i >= 0; i--) {
            double offset = Utils.normalRelativeAngle(absoluteBearing(myLocation, endPoints.get(i))
                    - directBearing);
    ...
    static ArrayList<Point2D.Double> getEndPoints(
        ...
        Collections.addAll(endPoints, getSmoothedEndPoints(enemyLocation, enemyHeading, enemyVelocity, myLocation, bulletPower, pointsList));
        Collections.addAll(endPoints, getDirectEndPoints(enemyLocation, enemyHeading, enemyVelocity, myLocation, bulletPower, endPoints, pointsList));
        Collections.addAll(endPoints, getStraightEndPoints(enemyLocation, enemyHeading, enemyVelocity, myLocation, bulletPower, pointsList));
    ...
    static Point2D.Double[] getSmoothedEndPoints(
        ...
        for (int i = -1; i < 2; i += 2) {
            ...
            while (bulletDistance < eloc.distance(myLocation)) {
                ...
                if (nextLoc.x > WIDTH - MARGIN || nextLoc.x < MARGIN
                        || nextLoc.y > HEIGHT - MARGIN || nextLoc.y < MARGIN)
                    break;
                eloc = nextLoc;
                pointsList.add(eloc);
